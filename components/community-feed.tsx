...
import { MessageSquare, Users, Calendar, Award, BookOpen, Briefcase } from "lucide-react"
    ...
    const interval = setInterval(() => {
      setCurrentIndex((prevIndex) => {
        ...
        if (endIndex > startIndex) {
          setVisibleActivities(activities.slice(startIndex, startIndex + 4))
        } else {
          setVisibleActivities([...activities.slice(startIndex), ...activities.slice(0, endIndex)])
        }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
      <div className="p-6 border-b border-slate-100">
        <h2 className="font-serif text-2xl mb-1">Live Community Feed</h2>
        <p className="text-black/60 font-light text-sm">Recent activities across our 14 city hubs</p>
      </div>
            ...
            {visibleActivities.map((activity) => (
              ...
              >
                <div
                  ...
                  ${activity.type === "opportunity" ? "bg-indigo-100 text-indigo-600" : ""}
                `}
                >
                  {activity.icon}
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-sm">{activity.city}</span>
                    <span className="text-xs text-black/50">•</span>
                    <span className="text-xs text-black/50">{activity.time}</span>
                  </div>
                  <p className="text-sm text-black/70">{activity.content}</p>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          {activities.map((_, index) => (
            <button
              ...
              className={`h-1 rounded-full transition-all ${
                index >= currentIndex && index < currentIndex + 4 ? "bg-black w-4" : "bg-black/20 w-2"
              }`}
